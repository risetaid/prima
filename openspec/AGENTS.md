...
# BAD - Never use git push for PRs
...
```bash
# GOOD - Use gh CLI for branch operations
gh repo clone                   # Clone repository
gh pr checkout [number]         # Switch to PR branch
gh pr merge [number]            # Merge PR

# Info-only operations
git branch --show-current       # ✅ OK to check current branch
git status                      # ✅ OK to check status
git diff                        # ✅ OK to review changes
```
...
- **Never use `git pull`** - Use `gh pr checkout` to switch to PR branches
- **Always review changes** - Run `git status` and `git diff` before any operations
- **Never use `--force`** flags - Protect repository history
- **Always confirm correct branch** - Check with `git branch --show-current` before operating
...
# 4. Switch to another PR (not git pull)
gh pr checkout [number]

# 5. Merge PR when done
gh pr merge [number]
```
