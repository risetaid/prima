 ...
 * Usage: pnpm scripts/apply-migration.ts
 *
...
async function main() {
  ...
  try {
    ...
    const migrationPath = join(
      ...
      "drizzle",
      "migrations",
      "0013_sticky_skreet_CONCURRENT.sql"
    );
    ...
    log(
      "\n⚠️  WARNING: This will create indexes in your database!",
      colors.yellow
    );
    ...
    if (!autoConfirm) {
      logWarning(
        "Dry run mode: Add --confirm flag to actually apply migration"
      );
      logInfo("\nExample: pnpm scripts/apply-migration.ts --confirm");
      await sql.end();
      process.exit(0);
    }
    ...
    for (let i = 0; i < statements.length; i++) {
      ...
      logInfo(
        `\n[${i + 1}/${statements.length}] Creating index: ${
          indexName || "unknown"
        }`
      );
    ...
    if (indexes.length > 0) {
      ...
      logWarning(
        "No indexes found - they may have been created with different names"
      );
    }

    // Show next steps
    log("\n📋 Next Steps:", colors.cyan);
    logInfo("1. Monitor index usage: pnpm scripts/check-index-usage.ts");
