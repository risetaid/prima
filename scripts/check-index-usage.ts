 ...
 * Usage: pnpm scripts/check-index-usage.ts
 *
...
async function main() {
  ...
  try {
    ...
    if (newIndexes.length === 0) {
      log(
        "⚠️  New indexes not found! Have you run the migration?",
        colors.yellow
      );
      log("   Run: pnpm scripts/apply-migration.ts --confirm", colors.blue);
    } else {
      newIndexes.forEach(
        (idx: {
          index_name: string;
          ...
          log(
            `${status} ${idx.table_name}.${idx.index_name}`,
            hasBeenUsed ? colors.green : colors.yellow
          );
      ...
      if (
        newIndexes.some((idx: { scans: string }) => parseInt(idx.scans) === 0)
      ) {
        log(
          "\n⏳ Some indexes have not been used yet. This is normal if:",
          colors.yellow
        );
        log("   • The indexes were just created", colors.yellow);
        log(
          "   • No queries have run yet that use these indexes",
          colors.yellow
        );
        log(
          "   • Query planner statistics need updating (run ANALYZE)",
          colors.yellow
        );
      }
    }
    ...
    tableStats.forEach(
      (table: {
        ...
        if (lastAnalyze) {
          const analyzeAge = Math.floor(
            (Date.now() - new Date(lastAnalyze).getTime()) / 1000 / 60
          );
          log(`   Last analyzed: ${analyzeAge} minutes ago`, colors.blue);
        } else {
          log(
            `   ⚠️  Never analyzed - run ANALYZE ${table.table_name}`,
            colors.yellow
          );
        }
        log("");
    ...
    try {
      ...
      if (result1 && result1.length > 0) {
        ...
        if (plan.includes("Index Scan")) {
          ...
          log(
            "   ⚠️  Using sequential scan (index not being used)",
            colors.yellow
          );
        }
      }
    ...
    log("\n💡 Recommendations\n", colors.cyan);
    log(
      "1. If indexes show 0 scans, run: ANALYZE conversation_states; ANALYZE conversation_messages;",
      colors.blue
    );
    log(
      "2. Monitor index usage over 24 hours to confirm they are being used",
      colors.blue
    );
    log(
      "3. Check /api/admin/migration-health for application-level metrics",
      colors.blue
    );
    log(
      "4. Enable DB_COMPOSITE_INDEXES feature flag if performance looks good",
      colors.blue
    );
  } catch (error) {
