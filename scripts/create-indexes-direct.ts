 ...
 * Usage: pnpm scripts/create-indexes-direct.ts --confirm
 */
...
async function main() {
  ...
  if (!autoConfirm) {
    log(
      "⚠️  Dry run mode: Add --confirm flag to create indexes",
      colors.yellow
    );
    log(
      "Example: pnpm scripts/create-indexes-direct.ts --confirm\n",
      colors.cyan
    );
    await sql.end();
    process.exit(0);
  ...
  try {
    ...
    log(
      "📊 Creating index 1/2: conversation_messages_state_created_idx",
      colors.cyan
    );
    ...
    log(
      "📊 Creating index 2/2: conversation_states_patient_active_expires_idx",
      colors.cyan
    );
    log("⏳ This may take several minutes...", colors.yellow);
    ...
    log("1. Run: pnpm scripts/analyze-tables.ts", colors.cyan);
    log("2. Check: pnpm scripts/check-index-usage.ts", colors.cyan);
