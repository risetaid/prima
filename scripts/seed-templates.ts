...
async function findSuperAdminUser(): Promise<any> {
  ...
  try {
    ...
    if (
      ...
      console.error(
        `   Status: active=${adminUser.isActive}, approved=${adminUser.isApproved}, role=${adminUser.role}`
      );
      throw new Error("Superadmin user not properly configured");
    }

    console.log(
      `✅ Superadmin found: ${adminUser.email} (${adminUser.firstName} ${adminUser.lastName})`
    );
    return adminUser;
  } catch (error) {
    console.error(
...
async function seedTemplates(adminUserId: string) {
  ...
  console.log(
    `   Skipped: ${skippedTemplates.length} templates (already exist)`
  );
...
async function seedTemplatesMain() {
  try {
    ...
    console.log(
      `   📱 Total available: ${stats.created + stats.skipped} templates`
    );
    console.log("");
    ...
    console.log(
      `   REMINDER: ${
        messageTemplates.filter((t) => t.category === "REMINDER").length
      } templates`
    );
    console.log(
      `   APPOINTMENT: ${
        messageTemplates.filter((t) => t.category === "APPOINTMENT").length
      } templates`
    );
    console.log(
      `   EDUCATIONAL: ${
        messageTemplates.filter((t) => t.category === "EDUCATIONAL").length
      } templates`
    );
