  ...
  const fetchPublishedArticles = async () => {
    try {
      ...
      const response = await fetch(
        "/api/content/public?type=articles&limit=20"
      );
      ...
      if (data.success && data.data) {
        ...
        const publishedArticles = data.data.filter(
          (item: Article) => "type" in item && item.type === "article"
        );
      ...
      setError(
        error instanceof Error
          ? error.message
          : "Gagal memuat artikel kesehatan"
      );
    } finally {
      setLoading(false);
    }
              ...
              <div className="mt-12 bg-gradient-to-r from-blue-50 to-indigo-50 rounded-xl p-8 border border-blue-100">
                <div className="text-center max-w-3xl mx-auto">
                  ...
                  <p className="text-gray-700 leading-relaxed">
                    Artikel-artikel ini telah disusun oleh tim medis
                    berpengalaman untuk memberikan informasi kesehatan yang
                    akurat dan mudah dipahami.
                    <strong className="text-blue-700">
                      {" "}
                      Selalu konsultasikan dengan dokter
                    </strong>{" "}
                    untuk mendapatkan saran medis yang tepat sesuai dengan
                    kondisi kesehatan Anda.
                  </p>
                  <div className="mt-6 flex flex-wrap justify-center gap-2">
                    <span className="px-3 py-1 bg-white rounded-full text-sm text-blue-700 border border-blue-200">
            ...
            {filteredArticles.map((article) => {
              ...
              return (
                <div
                  key={article.id}
                  onClick={() =>
                    window.open(`/content/articles/${article.slug}`, "_blank")
                  }
