    ...
    <>
      ...
      <div className="hidden lg:flex items-center space-x-3 mb-6">
        ...
        <div className="flex items-center space-x-2">
          ...
          {patient && (
            <span className="text-lg text-gray-600">- {patient.name}</span>
          )}
        </div>
      </div>
...
function ActionButtons({
  ...
  return (
    <div className="flex gap-4 lg:static lg:mt-8 fixed bottom-6 left-4 right-4 lg:bottom-auto lg:left-auto lg:right-auto lg:flex-row flex-col lg:max-w-none max-w-none">
      {isDeleteMode ? (
        <>
          <button
            onClick={onCancel}
...
export default function PatientSymptomsPage() {
  ...
  const [editingSymptom, setEditingSymptom] = useState<PatientSymptom | null>(
    null
  );
  ...
  const handleDelete = async () => {
    ...
    const confirmed = await new Promise<boolean>((resolve) => {
      toast.warning(`Hapus ${selectedSymptoms.length} Gejala?`, {
        description:
          "Tindakan ini tidak dapat dibatalkan. Data gejala akan dihapus permanen.",
    ...
    try {
      ...
      if (response.ok) {
        setSymptoms((prev) =>
          prev.filter((s) => !selectedSymptoms.includes(s.id))
        );
        setSelectedSymptoms([]);
...
function AddSymptomDialog({
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      setFormData({
        symptomText: "",
        recordedAt: wibTime.toISOString().slice(0, 16),
      });
    }
  }, [isOpen]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(
        `/api/patients/${patientId}/symptoms/${symptom.id}`,
        {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify(formData),
