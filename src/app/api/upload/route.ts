  ...
  try {
    ...
    if (type === 'general') {
      ...
      if (file.size > sizeLimit) {
        ...
        return NextResponse.json({ error: `File size must be less than ${limitMB}MB` }, { status: 400 })
      }
    ...
    if (type !== 'general') {
      ...
      while (attempts < maxAttempts) {
        try {
          await minioClient.putObject(bucketName, currentFilename, buffer, buffer.length, {
    ...
    if (type === 'tinymce-image') {
      return NextResponse.json({ location: publicUrl }, { headers: corsHeaders })
