...
// Components
function PageHeader({ patient, onBack }: { patient?: Patient | null; onBack: () => void }) {
  return (
    <>
      {/* Mobile Header */}
      <header className="lg:hidden bg-white">
        <div className="flex justify-between items-center px-4 py-4">
          <button onClick={onBack} className="p-1 hover:bg-gray-100 rounded-full cursor-pointer">
...
function SymptomItem({ symptom, isDeleteMode, isSelected, onToggle, onEdit }: {
  ...
  return (
    <div className="flex items-center space-x-3">
      ...
      <div
        ...
        onClick={isDeleteMode ? undefined : onEdit}
      >
...
function ActionButtons({ isDeleteMode, symptomsLength, selectedCount, onAdd, onDeleteMode, onCancel, onDelete }: {
  isDeleteMode: boolean
...
export default function PatientSymptomsPage() {
  ...
  const fetchData = async (id: string) => {
    try {
      const [patientRes, symptomsRes] = await Promise.all([
        ...
        fetch(`/api/patients/${id}/symptoms`)
      ])
  ...
  const handleDelete = async () => {
    ...
    const confirmed = await new Promise<boolean>((resolve) => {
      toast.warning(`Hapus ${selectedSymptoms.length} Gejala?`, {
        ...
        action: { label: 'Hapus', onClick: () => resolve(true) },
    ...
    try {
      ...
      if (response.ok) {
        ...
        toast.error('Gagal Menghapus', { description: 'Tidak dapat menghapus gejala. Coba lagi.' })
      }
    } catch (error) {
    ...
    return (
      <div className="min-h-screen bg-gray-50 relative">
        <div className="absolute inset-0 overflow-hidden pointer-events-none z-0">
          <div className="fixed inset-0 bg-cover bg-center bg-no-repeat opacity-90" style={{ backgroundImage: "url(/bg_desktop.png)" }} />
        </div>
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <PageHeader patient={patient} onBack={() => router.back()} />
        ...
        <div className="space-y-3 mb-8">
          {symptoms.map((symptom) => (
            <SymptomItem
              ...
              onToggle={() => toggleSelection(symptom.id)}
              onEdit={() => {
...
function AddSymptomDialog({ isOpen, patientId, onClose, onSuccess }: AddSymptomDialogProps) {
  const [formData, setFormData] = useState({ symptomText: '', recordedAt: '' })
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      setFormData({ symptomText: '', recordedAt: wibTime.toISOString().slice(0, 16) })
    }
  }, [isOpen])
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      else toast.error('Gagal Menambah Gejala', { description: 'Tidak dapat menambahkan gejala baru. Coba lagi.' })
    } catch (error) {
    ...
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-2xl p-6 w-full max-w-sm mx-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="block text-gray-600 text-sm mb-2">Tanggal & Waktu</label>
            <input type="datetime-local" value={formData.recordedAt} onChange={(e) => setFormData({ ...formData, recordedAt: e.target.value })} className="w-full px-4 py-3 border-2 border-blue-200 rounded-xl focus:border-blue-500 focus:outline-none transition-colors" required />
          </div>
          <div>
            <label className="block text-gray-600 text-sm mb-2">Gejala</label>
            <textarea value={formData.symptomText} onChange={(e) => setFormData({ ...formData, symptomText: e.target.value })} placeholder="Masukkan gejala yang dialami pasien..." className="w-full px-4 py-3 border-2 border-blue-200 rounded-xl focus:border-blue-500 focus:outline-none transition-colors resize-none" rows={3} required />
          </div>
          <div className="flex space-x-3 pt-4">
            <button type="button" onClick={onClose} className="flex-1 bg-gray-200 text-gray-700 py-3 rounded-xl font-medium hover:bg-gray-300 transition-colors">Batal</button>
            <button type="submit" disabled={loading} className="flex-1 bg-blue-500 text-white py-3 rounded-xl font-medium hover:bg-blue-600 transition-colors disabled:opacity-50">{loading ? 'Loading...' : 'Simpan'}</button>
          </div>
        </form>
      </div>
    </div>
...
function EditSymptomDialog({ isOpen, symptom, patientId, onClose, onSuccess }: EditSymptomDialogProps) {
  ...
  useEffect(() => {
    if (isOpen && symptom) {
      ...
      setFormData({ symptomText: symptom.symptomText, recordedAt: wibTime.toISOString().slice(0, 16) })
    }
  }, [isOpen, symptom])
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) onSuccess()
      else toast.error('Gagal Update Gejala', { description: 'Tidak dapat memperbarui data gejala. Coba lagi.' })
    } catch (error) {
  ...
  return (
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-2xl p-6 w-full max-w-sm mx-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="block text-gray-500 text-sm mb-2">Waktu</label>
          ...
          <div>
            <label className="block text-gray-500 text-sm mb-2">Gejala Pasien</label>
            <input type="text" value={formData.symptomText} onChange={(e) => setFormData({ ...formData, symptomText: e.target.value })} placeholder="Gatal-gatal, benjolan" className="w-full px-4 py-3 border-2 border-blue-200 rounded-xl focus:border-blue-500 focus:outline-none transition-colors" required />
          </div>
          <div className="flex space-x-3 pt-4">
            <button type="button" onClick={onClose} className="flex-1 bg-gray-200 text-gray-700 py-3 rounded-xl font-medium hover:bg-gray-300 transition-colors">Batal</button>
            <button type="submit" disabled={loading} className="flex-1 bg-blue-500 text-white py-3 rounded-xl font-medium hover:bg-blue-600 transition-colors disabled:opacity-50">{loading ? 'Loading...' : 'Simpan'}</button>
