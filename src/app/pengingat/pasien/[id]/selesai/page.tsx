  ...
  const fetchCompletedReminders = async (patientId: string, page: number) => {
    try {
      const response = await fetch(
        `/api/patients/${patientId}/reminders?filter=completed&page=${page}&limit=10`
      );
      if (response.ok) {
        ...
        if (
          ...
          logger.info("✅ Completed reminders response (paginated):", {
            page,
            operation: "fetch-completed",
          });
          const normalized = result.data.data.map(
            (item: CompletedReminder) => ({
              ...item,
        ...
        if (result.data && result.pagination && Array.isArray(result.data)) {
          logger.info("✅ Completed reminders response (direct paginated):", {
            page,
            operation: "fetch-completed",
          });
          const normalized = result.data.map((item: CompletedReminder) => ({
            ...item,
            manuallyConfirmed: Boolean(item.manuallyConfirmed),
          }));
          setReminders(normalized);
              ...
              <div className="bg-gray-50 p-4">
                <div className="flex-1">
                  ...
                  <p className="text-gray-600 text-sm mb-1">
                    Dikirim pada:{" "}
                    {formatShortDateTime(
                      reminder.sentAt || reminder.reminderDate
                    )}
                  </p>
