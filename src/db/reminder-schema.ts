...
export const reminders = pgTable(
  ...
  (table) => ({
    ...
    patientActiveIdx: index("reminders_patient_active_idx").on(
      table.patientId,
      table.isActive,
    ),
    patientStatusIdx: index("reminders_patient_status_idx").on(
      table.patientId,
      table.status,
    ),
    todayRemindersIdx: index("reminders_today_idx").on(
      table.startDate,
      table.isActive,
      table.scheduledTime,
    ),
    patientTypeIdx: index("reminders_patient_type_idx").on(
      table.patientId,
      table.reminderType,
    ),
...
export const manualConfirmations = pgTable(
  "manual_confirmations",
  {
    ...
    confirmationType: text("confirmation_type", {
      enum: ["VISIT", "PHONE_CALL", "MESSAGE", "GENERAL"],
    })
      .notNull()
      .default("GENERAL"),
  ...
  (table) => ({
    ...
    patientVolunteerIdx: index("manual_confirmations_patient_volunteer_idx").on(
      table.patientId,
      table.volunteerId,
    ),
    // Standalone index for reminder lookups (fixes N+1 in ComplianceService)
    reminderIdIdx: index("manual_confirmations_reminder_id_idx").on(
      table.reminderId,
    ),
    // Foreign keys
    ...
    // Note: Added reminderId index to optimize batch queries in ComplianceService
    // patientId and volunteerId covered by composite index
    // Other single-column indexes removed (reminderType, confirmationType, visitDate, confirmedAt)
  }),
);
...
export const reminderLogs = pgTable(
  ...
  (table) => ({
    ...
    reminderActionIdx: index("reminder_logs_reminder_action_idx").on(
      table.reminderId,
      table.action,
    ),
    patientTimestampIdx: index("reminder_logs_patient_timestamp_idx").on(
      table.patientId,
      table.timestamp,
    ),
...
export const conversationStates = pgTable(
  "conversation_states",
  {
    ...
    lastClarificationSentAt: timestamp("last_clarification_sent_at", {
      withTimezone: true,
    }),
  ...
  (table) => ({
    ...
    patientActiveExpiresIdx: index(
      "conversation_states_patient_active_expires_idx",
    ).on(table.patientId, table.isActive, table.expiresAt),
    // Partial index for cleanup cron job (only indexes active, non-deleted conversations)
    // 50% smaller than full index, optimized for WHERE is_active = true AND deleted_at IS NULL
    // Note: Defined in migration SQL, not in Drizzle (partial index support limited)
...
export const conversationMessages = pgTable(
  ...
  (table) => ({
    ...
    stateCreatedIdx: index("conversation_messages_state_created_idx").on(
...
export const volunteerNotifications = pgTable(
  ...
  (table) => ({
    patientIdIdx: index("volunteer_notifications_patient_id_idx").on(
      table.patientId,
    ),
    // Foreign key to patients
    patientIdFk: foreignKey({
...
export type NewVolunteerNotification =
  typeof volunteerNotifications.$inferInsert;
