...
export class MedicalQueryService {
  ...
  async getPatientsWithCompliance(
    ...
    if (filters.assignedVolunteerId) {
      whereConditions.push(
        eq(patients.assignedVolunteerId, filters.assignedVolunteerId)
      );
    }
    ...
    // Combine data
    const result: PatientWithVolunteer[] = patientsWithVolunteers.map(
      (patient) => ({
        id: patient.id,
        name: patient.name,
   ...
   * Simplified: Only counts "Selesai" (completed) reminders
   */
  async getComplianceRatesBatch(
    patientIds: string[]
  ): Promise<Map<string, ComplianceData>> {
    if (patientIds.length === 0) {
      return new Map();
    }

    // Use the simplified compliance calculation from PatientRepository
    const complianceData = await this.getCompletedComplianceCounts(patientIds);

    const complianceMap = new Map<string, ComplianceData>();
    complianceData.forEach((data) => {
      complianceMap.set(data.patientId, {
        patientId: data.patientId,
        deliveredReminders: data.totalConfirmed, // Now represents total confirmed
        confirmedReminders: data.takenCount, // Now represents taken count
        complianceRate:
          data.totalConfirmed > 0
            ? Math.round((data.takenCount / data.totalConfirmed) * 100)
            : 0,
        missedReminders: data.totalConfirmed - data.takenCount,
      });
    });
  ...
  /**
   * Get completed compliance counts (helper method)
   * Only counts reminders that have been confirmed (manual or automated)
   */
  private async getCompletedComplianceCounts(patientIds: string[]) {
    // Count manual confirmations where medication was taken
    const manualTaken = await db
      .select({
        patientId: manualConfirmations.patientId,
        count: count().as("taken_count"),
      })
      .from(manualConfirmations)
      .where(
        and(
          inArray(manualConfirmations.patientId, patientIds),
    ...
    // Total confirmed = manual + automated
    const totalConfirmed = await db
      ...
      .leftJoin(
        reminderLogs,
        and(
          eq(manualConfirmations.patientId, reminderLogs.patientId),
          eq(reminderLogs.confirmationStatus, "CONFIRMED")
        )
      )
    ...
    manualTaken.forEach((row) => {
      takenMap.set(
        row.patientId,
        (takenMap.get(row.patientId) || 0) + Number(row.count)
      );
    });

    automatedTaken.forEach((row) => {
      takenMap.set(
        row.patientId,
        (takenMap.get(row.patientId) || 0) + Number(row.count)
      );
    ...
    totalConfirmed.forEach((row) => {
      totalMap.set(row.patientId, Number(row.count));
    });

    // Combine results
    const results = patientIds.map((patientId) => ({
      patientId,
      totalConfirmed: totalMap.get(patientId) || 0,
      takenCount: takenMap.get(patientId) || 0,
    }));
  ...
  async getReminderStats(patientId: string): Promise<ReminderStats> {
    ...
    const stats = await db
      .select({
        ...
        pending: count(
          sql`CASE WHEN ${reminderLogs.status} = 'PENDING' THEN 1 END`
        ).as("pending"),
        sent: count(
          sql`CASE WHEN ${reminderLogs.status} = 'SENT' THEN 1 END`
        ).as("sent"),
        delivered: count(
          sql`CASE WHEN ${reminderLogs.status} = 'DELIVERED' THEN 1 END`
        ).as("delivered"),
        failed: count(
          sql`CASE WHEN ${reminderLogs.status} = 'FAILED' THEN 1 END`
        ).as("failed"),
        todayDelivered: count(
          sql`CASE WHEN ${reminderLogs.status} = 'DELIVERED' AND DATE(${reminderLogs.sentAt}) = CURRENT_DATE THEN 1 END`
        ).as("today_delivered"),
        thisWeekDelivered: count(
          sql`CASE WHEN ${reminderLogs.status} = 'DELIVERED' AND ${reminderLogs.sentAt} >= CURRENT_DATE - INTERVAL '7 days' THEN 1 END`
        ).as("week_delivered"),
      })
      .from(reminderLogs)
  ...
  async getPatientRemindersWithSchedule(
    patientId: string,
    filters: {
      status?: "pending" | "completed" | "scheduled" | "all";
      limit?: number;
    } = {}
  ) {
  ...
  async getUsersWithApprovalInfo(includeInactive = false) {
    ...
    const approver = db.$with("approver").as(db.select().from(users));
    ...
    const patientStats = await db
      .select({
        ...
        activePatients: count(
          sql`CASE WHEN ${patients.isActive} = true THEN 1 END`
        ).as("active"),
      })
      .from(patients)
    ...
    const reminderStats = await db
      .select({
        ...
        todayReminders: count(
          sql`CASE WHEN DATE(${reminderLogs.sentAt}) = CURRENT_DATE THEN 1 END`
        ).as("today"),
      })
      .from(reminderLogs)
      .leftJoin(patients, eq(reminderLogs.patientId, patients.id))
      .where(
        reminderWhereConditions.length > 0
          ? and(...reminderWhereConditions)
          : undefined
      );
...
export function calculateIndonesianCompliance(
  ...
  const baseRate =
    delivered > 0 ? Math.round((confirmed / delivered) * 100) : 0;
...
export async function measureMedicalQuery<T>(
  ...
  try {
    ...
    if (duration > 1000) {
      console.warn(
        `🐌 Slow Medical Query [${queryName}] took ${duration}ms - consider optimization`
      );
    }
    ...
    const duration = Date.now() - startTime;
    console.error(
      `❌ Medical Query [${queryName}] failed after ${duration}ms:`,
      error
    );
    throw error;
  }
