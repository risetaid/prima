...
export class MedicationFallbackResolver {
   ...
   */
  resolveMedicationName(context: MedicationResolutionContext): MedicationResolutionResult {
    // Try strategies in order of preference
    for (const strategy of this.strategies) {
      if (strategy.applicable(context) && strategy.level <= this.maxFallbackLevel) {
  ...
  getMedicationSuggestions(
    ...
    const phoneticMatches = this.getPhoneticMatches(normalizedInput, currentMedications)
   ...
   */
  validateMedicationQuality(medication: MedicationDetails): MedicationQualityScore {
    const issues: string[] = []
    const suggestions: string[] = []
    let score = 1.0
    ...
    if (!medication.name || medication.name.length < 2) {
      ...
      score -= 0.3
    } else if (medication.name.toLowerCase() === 'obat' || medication.name.toLowerCase() === 'unknown') {
    ...
    if (medication.category === 'OTHER') {
      suggestions.push('Pertimbangkan untuk mengkategori obat dengan lebih spesifik')
      score -= 0.1
    }
   ...
   */
  generateContextualMedicationName(context: MedicationResolutionContext): string {
    ...
    const medicationVariable = patientVariables.find(v =>
      v.category === 'MEDICATION' && v.variableName === 'obat'
    )
  ...
  private hasExactMatch(context: MedicationResolutionContext): boolean {
    ...
    return context.currentMedications.some(med =>
      med.name.toLowerCase() === input
    )
  }

  private resolveExactMatch(context: MedicationResolutionContext): MedicationResolutionResult {
    const input = context.originalInput.toLowerCase().trim()
    const medication = context.currentMedications.find(med =>
      med.name.toLowerCase() === input
    )!
  ...
  private hasPartialMatch(context: MedicationResolutionContext): boolean {
    const input = context.originalInput.toLowerCase().trim()
    return context.currentMedications.some(med =>
      med.name.toLowerCase().includes(input) || input.includes(med.name.toLowerCase())
    )
  }

  private resolvePartialMatch(context: MedicationResolutionContext): MedicationResolutionResult {
    const input = context.originalInput.toLowerCase().trim()
    const matches = context.currentMedications.filter(med =>
      med.name.toLowerCase().includes(input) || input.includes(med.name.toLowerCase())
    )
  ...
  private hasMedicationVariables(context: MedicationResolutionContext): boolean {
    return context.patientVariables.some(v => v.category === 'MEDICATION')
  }

  private resolveFromVariables(context: MedicationResolutionContext): MedicationResolutionResult {
    const medicationVars = context.patientVariables.filter(v => v.category === 'MEDICATION')
  ...
  private canApplyPhoneticMatching(context: MedicationResolutionContext): boolean {
    return context.originalInput.length >= 3
  }

  private resolvePhoneticMatch(context: MedicationResolutionContext): MedicationResolutionResult {
  ...
  private resolveContextual(context: MedicationResolutionContext): MedicationResolutionResult {
  ...
  private createDefaultFallback(_context: MedicationResolutionContext): MedicationResolutionResult {
    return {
      success: true,
      medicationName: 'obat',
      confidence: 0.3,
  ...
  private getPhoneticMatches(input: string, _medications: MedicationDetails[]): string[] {
    ...
    for (const [canonical, variants] of Object.entries(phoneticMap)) {
      if (variants.some(variant => variant.includes(input))) {
  ...
  private generateFromContextClues(context: MedicationResolutionContext): string {
 ...
 */
export function getMedicationQualityScore(medication: MedicationDetails): MedicationQualityScore {
  const resolver = createMedicationFallbackResolver()
