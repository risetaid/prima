...
export class ReminderTesting {
  ...
  static async createTestReminder(
    ...
    const [reminder] = await db.insert(reminders).values(testData).returning();
  ...
  static async createTestManualConfirmation(
    overrides: Partial<{
      patientId: string;
      reminderType: "MEDICATION" | "APPOINTMENT" | "GENERAL";
  ...
  static async validateSchemaStructure(): Promise<{
    ...
    try {
      ...
      const reminderColumnNames = reminderColumns.map(
        (col) => (col as { column_name: string }).column_name
      );
      ...
      const actualTypes = enumResult.map(
        (row) => (row as { enumlabel: string }).enumlabel
      );
  ...
  static async testDataIntegrity(): Promise<{
    ...
    try {
      ...
      `);
      stats.orphanedLogs =
        Number((orphanedLogs[0] as { count: number })?.count) || 0;
      ...
      `);
      stats.orphanedConfirmations =
        Number((orphanedConfirmations[0] as { count: number })?.count) || 0;

      if (stats.orphanedConfirmations > 0) {
        issues.push(
          `Found ${stats.orphanedConfirmations} orphaned manual confirmations`
        );
      }
      ...
      const invalidReminders = await db.execute(sql`
        ...
        WHERE
          ...
          start_date IS NULL
      `);
      stats.invalidReminders =
        Number((invalidReminders[0] as { count: number })?.count) || 0;

      if (stats.invalidReminders > 0) {
        issues.push(
          `Found ${stats.invalidReminders} reminders with missing required fields`
        );
      }
      ...
      stats.futureReminders =
        Number((futureReminders[0] as { count: number })?.count) || 0;
      ...
      `);
      stats.noEndDateReminders =
        Number((noEndDateReminders[0] as { count: number })?.count) || 0;
  ...
  static async testMigrationDryRun(): Promise<{
    ...
    try {
      ...
      if (!hasLogs) {
        warnings.push(
          "reminder_logs table does not exist - will be created by migration"
        );
      }

      if (!hasMigrations) {
        warnings.push(
          "drizzle_migrations table does not exist - first migration needed"
        );
      }
      ...
      const typeList = currentTypes.map(
        (row) => (row as { enumlabel: string }).enumlabel
      );
      if (!typeList.includes("MEDICATION")) {
        warnings.push(
          "MEDICATION type not found in enum - will be added by migration"
        );
      }
      ...
      const enhancedColumns = columnCheck.map(
  ...
  static async runAllTests(): Promise<{
    ...
    logger.info("🧪 Running comprehensive reminder system tests...");
  ...
  static async cleanupTestData(): Promise<void> {
    ...
    await db.delete(manualConfirmations).where(sql`patient_id LIKE 'test-%'`);

    await db.delete(reminders).where(sql`patient_id LIKE 'test-%'`);
