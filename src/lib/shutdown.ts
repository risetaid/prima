...
async function shutdown(signal: string) {
  ...
  try {
    ...
    try {
      // Redis client will close automatically on process exit
      // No explicit disconnect method available in wrapper
      logger.info('Redis connection will close on process exit');
    } catch (error) {
