...
Lakukan secara rutin untuk hasil optimal.`,
    videoUrl: "https://www.youtube.com/embed/inpok4MKVLM", // Sample breathing exercise video
...
Lakukan 2-3 kali sehari untuk menjaga fleksibilitas tubuh.`,
    videoUrl: "https://www.youtube.com/embed/g_tea8ZNk5A", // Sample bed exercises video
...
Video ini dapat ditonton bersama keluarga untuk saling menguatkan.`,
    videoUrl: "https://www.youtube.com/embed/Kb24RrHIbFk", // Sample motivational video
...
export async function seedCMSContent() {
  console.log('🌱 Starting to seed CMS content...')

  try {
    // Insert sample articles
    console.log('📄 Inserting sample articles...')
    for (const article of sampleArticles) {
      const result = await db.insert(cmsArticles).values(article).returning()
      console.log(`✅ Created article: ${result[0].title}`)
    }

    // Insert sample videos
    console.log('🎬 Inserting sample videos...')
    for (const video of sampleVideos) {
      const result = await db.insert(cmsVideos).values(video).returning()
      console.log(`✅ Created video: ${result[0].title}`)
    }
