...
import { CONVERSATION_SYSTEM_PROMPT, buildConversationPrompt } from './ai-prompts';
import { logger } from '@/lib/logger';
import type { AIConversationContext, AIConversationResponse } from '@/lib/ai-types';
...
export class AIConversationService {
  ...
  async respond(
    ...
    try {
      ...
      const transformedPatientContext = context.patientContext ? {
        ...
        recentReminders: context.patientContext.recentReminders?.map(r => r.message)
      } : undefined;
      ...
      const shouldEscalate = this.detectEscalation(content, message);
      const escalationReason = shouldEscalate
        ? this.extractEscalationReason(content, message)
        : undefined;
      ...
      logger.error('Failed to generate AI conversation response', error as Error, {
        patientId: context.patientId,
   ...
   */
  private detectEscalation(aiResponse: string, patientMessage: string): boolean {
  ...
  private extractEscalationReason(
    ...
    if (response.includes('hubungi dokter') || response.includes('konsultasi')) {
