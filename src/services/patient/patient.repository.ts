...
import { db, patients, patientVariables, healthNotes, users, reminderLogs, manualConfirmations, medications, patientMedications, verificationLogs, reminderSchedules } from '@/db'
import { and, eq, isNull, inArray, desc, count, sql } from 'drizzle-orm'
import type { PatientVariableRow, NewPatientVariableRow, PatientFilters } from './patient.types'
...
export class PatientRepository {
  ...
  async getActiveVariables(patientId: string): Promise<PatientVariableRow[]> {
    return await db
      ...
      .where(and(eq(patientVariables.patientId, patientId), eq(patientVariables.isActive, true), isNull(patientVariables.deletedAt)))
      .orderBy(patientVariables.variableName)
  }

  async upsertVariables(patientId: string, variables: Record<string, string>, createdById: string): Promise<void> {
    // Read existing to decide update vs insert
    const existing = await db
      .select({ variableName: patientVariables.variableName })
      .from(patientVariables)
      .where(and(eq(patientVariables.patientId, patientId), eq(patientVariables.isActive, true)))
    ...
    for (const [name, value] of Object.entries(variables)) {
      ...
      if (existingSet.has(name)) {
        ops.push(
          db.update(patientVariables)
            .set({ variableValue: val, updatedAt: new Date() })
            .where(and(eq(patientVariables.patientId, patientId), eq(patientVariables.variableName, name), eq(patientVariables.isActive, true)))
        )
      } else {
        ops.push(
  ...
  async softDeleteVariable(patientId: string, variableName: string): Promise<number> {
    const result = await db
      .update(patientVariables)
      .set({ deletedAt: new Date(), updatedAt: new Date(), isActive: false })
      .where(and(eq(patientVariables.patientId, patientId), eq(patientVariables.variableName, variableName), eq(patientVariables.isActive, true)))
  ...
  async listHealthNotes(patientId: string) {
    return await db
      ...
      .from(healthNotes)
      .where(and(eq(healthNotes.patientId, patientId), isNull(healthNotes.deletedAt)))
  ...
  async getHealthNote(patientId: string, noteId: string) {
    const rows = await db
      ...
      .from(healthNotes)
      .where(and(eq(healthNotes.id, noteId), eq(healthNotes.patientId, patientId), isNull(healthNotes.deletedAt)))
      .limit(1)
    return rows[0] || null
  }

  async createHealthNote(patientId: string, note: string, noteDate: Date, recordedBy: string) {
  ...
  async softDeleteHealthNote(patientId: string, noteId: string) {
    const deleted = await db
      ...
      .where(and(eq(healthNotes.id, noteId), eq(healthNotes.patientId, patientId)))
  ...
  async softDeleteHealthNotes(patientId: string, noteIds: string[]) {
    ...
    const deleted = await db
      .update(healthNotes)
      .set({ deletedAt: new Date(), updatedAt: new Date() })
      .where(and(inArray(healthNotes.id, noteIds), eq(healthNotes.patientId, patientId)))
      .returning({ id: healthNotes.id })
    return deleted.length
  }

  async getUsersByIds(userIds: string[]) {
    if (!userIds.length) return [] as Array<{ id: string; firstName: string | null; lastName: string | null; email: string }>
    return await db
  ...
  async getUserById(id: string) {
    const rows = await db
      .select({ id: users.id, isActive: users.isActive, firstName: users.firstName, lastName: users.lastName, email: users.email })
      .from(users)
  ...
  async listPatients(filters: PatientFilters) {
    ...
    if (assignedVolunteerId) conditions.push(eq(patients.assignedVolunteerId, assignedVolunteerId))
  ...
  // ===== Compliance counts =====
  async getDeliveredCounts(patientIds: string[]) {
    if (!patientIds.length) return [] as Array<{ patientId: string; count: number }>
    const rows = await db
      .select({ patientId: reminderLogs.patientId, count: count(reminderLogs.id).as('delivered_count') })
      .from(reminderLogs)
      .where(and(inArray(reminderLogs.patientId, patientIds), eq(reminderLogs.status, 'DELIVERED')))
      .groupBy(reminderLogs.patientId)
    // Drizzle count returns a bigint-like; coerce to number
    return rows.map(r => ({ patientId: r.patientId, count: Number(r.count as unknown as string) }))
  }

  async getConfirmationsCounts(patientIds: string[]) {
    if (!patientIds.length) return [] as Array<{ patientId: string; count: number }>
    const rows = await db
      .select({ patientId: manualConfirmations.patientId, count: count(manualConfirmations.id).as('confirmations_count') })
  ...
  async insertVerificationLog(values: { patientId: string; action: string; patientResponse?: string | null; verificationResult?: any; processedBy?: string | null }) {
